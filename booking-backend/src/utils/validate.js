...
const validatePayload = {
  validateTripPayload: async (
    ...
    //
    if (originStationId) {
      const origin = await prisma.station.findUnique({
    ...
    //
    if (destStationId) {
      const dest = await prisma.station.findUnique({
        where: { id: destStationId },
      });
      if (!dest)
        throwError("Not Found: Destination station does not exist", 404);
    }
    //
    ...
    if (originStationId && destStationId && originStationId === destStationId) {
      throwError(
        "Bad Request: Origin and destination stations cannot be the same",
        400,
      );
    }
    ...
    if (originStationId && !destStationId && existing) {
      if (originStationId === existing.destStationId) {
        throwError(
          "Bad Request: Origin station cannot be the same as destination station",
          400,
        );
      }
    }
    ...
    if (!originStationId && destStationId && existing) {
      if (destStationId === existing.originStationId) {
        throwError(
          "Bad Request: Destination station cannot be the same as origin station",
          400,
        );
      }
    }
    ...
    if (departureTime && arrivalTime) {
      ...
      if (arr <= dep) {
        throwError(
          "Bad Request: Arrival time must be after departure time",
          400,
        );
      }
    }
